...
VERY MUCH WORK IN PROGRESS - ABSORBED FROM MASTODON
...
\begin{enumerate}
\item $\bm J_k \cdot \delta \vec X_k = -\vec F(\vec X_k)$, or in the 
...
\item $\vec X_{k+1} = \vec X_k + \alpha_k \delta \vec X_k$
\end{enumerate}
...
\]
so that step 1 of the above iterations become:
\[
\left(
\begin{array}{cc}
\K_k & \G \\ \G^T & 0
