...
is subjected to a unidirectional tensile load of $P$ in the $x$ direction as shown
in the figure. In this case, only one quarter of the domain is analysed due
to symmetry along $x$ and $y$ axis. 

\begin{center}
\includegraphics[width=0.8\textwidth]{images/benchmark_hole/rama16}\\
{\scriptsize Setup for the infinite plate with a circular hole benchmark.}
\end{center}


The analytical stress components for this
problem are \cite{chnn10}

\begin{eqnarray}
\sigma_{xx}(x,y) &=& P \left(  1-\frac{a^2}{r^2}(\frac{3}{2}\cos 2\theta + \cos 4\theta) 
+ \frac{3a^4}{2r^4} \cos 4\theta \right) \\
\sigma_{yy}(x,y) &=& -P \left( \frac{a^2}{r^2} (\frac{3}{2}\cos 2\theta - \cos 4\theta) 
- \frac{3a^4}{2r^4} \cos 4\theta \right) \\
\sigma_{xy}(x,y) &=& -P \left( \frac{a^2}{r^2} (\frac{3}{2}\sin 2\theta + \sin 4\theta) 
+ \frac{3a^4}{2r^4} \sin 4\theta \right) 
\end{eqnarray}

CHECK FORMULAS - never been implemented yet!
...
Young modulus 3e7 MPa, Poisson Ratio 0.3, Load $P$=10 N/m$^2$, a=1.
