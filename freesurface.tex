...
Note that Rose et al (2017) \cite{robh17} go further than this, 
...
Note that Rose et al (2017) \cite{robh17} go further than this and 
...
Duretz \etal \cite{dumg11} is concerned with the Finite Difference method so I will 
...
at about the same time. The third and fourth paper implement the algorithm in \aspect{} and 
FAIStokes respectively. 

The basic idea is rather simple. As stated in \cite{qube11}: 
...
I first present results from my doomed 2014 paper \cite{thie14} 
in which I carried out the Rayleigh-Taylor 
instability experiment described in \cite{kamm10}
where a dense fluid overlays another fluid 
in a square domain with their initial interface being of sinusoidal shape\footnote{This 
...
{\captionfont 
When the time step $\delta t$ is small, the simulation evolves smoothly, but 
...
{\captionfont The stabilisation algorithm is first switched off and up to a time step of $\delta t\sim 3800\si{\year}$, 
the free surface does not develop any instability (all curves are superimposed). 
For a time step of $3900\si{\year}$ and above, a clear oscillation develops (green seesaw curve on the figure).
Turning the algorithm on, time steps larger than $dt\sim 10,000\si{\year}$ can be used and no oscillation is observed. }
\end{center}
...
\Literature: Furuchi \cite{furu11}
