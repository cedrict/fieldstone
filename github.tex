...
Check it with \verb'git config --list'
...
I have created a simple bash script ({\tt get\_aspect}) that does this above in one go:
\begin{verbatim}
git clone git@github.com:geodynamics/aspect.git 
cd aspect
git remote rename origin upstream
git remote add cedrict git@github.com:cedrict/aspect.git
git remote -v
\end{verbatim}
...
git log : gives history of comments/changes/authors of a file


\newpage
\underline{how to squash/fixup:}

\begin{itemize}
\item \verb'git rebase main'
\item \verb'git rebase -i main'. A similar window as this will open:

\begin{center}
\includegraphics[width=13cm]{images/github/aaa}
\end{center}

\item Since we wish to squash (or rather fixup!) we do (replace all pick by f except first):

\begin{center}
\includegraphics[width=13cm]{images/github/bbb}
\end{center}

\item save and exit :wq
\item do \verb'git diff' (not necessary)
\item \verb'git push -f cedrict mybranch' 
\item all is well now
\end{itemize}
