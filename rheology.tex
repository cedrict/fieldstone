...
The code used is in {\tt images/rheology/surfaces}.
...
\includegraphics[width=10cm]{images/rheology/surfaces/surfaces_xy.pdf}

The second one is in the plane $\sigma_1+\sigma_2+\sigma_3=0$ which is perpendicular to the middle line 
$\sigma_1=\sigma_2=\sigma_3=0$. To ease the plot the points are plotted as a function of $\sigma_1$:

\includegraphics[width=10cm]{images/rheology/surfaces/surfaces_plane.pdf}

We see that we indeed recover that the two Drucker-Prager formulations inscribe and circumscribe the 
Mohr-Coulomb one. 
...
Review article from 1966: Guyot \& Dorn \cite{gudo66}
...
\Literature \cite{grev97,krab16,grbl09,issg15}
...
\index{general}{Elasto-Visco-Plasticity}
...
In what follows I present a few examples of configurations.

\input{tikz_dashpotdashpot}

\begin{itemize}
\item two viscous dampers in series: each is subjected to the same stress $\tau$ but deforms 
...
\[
\frac{tau}{2\dot{\varepsilon}_T} = \frac{\tau}{2\eta_1} + \frac{\tau}{2\eta_2}
\]
or, 
\[
...
\item two viscous dampers in parallel: each is deforms with the same strain rate $\dot{\varepsilon}_T$
...
\item on viscous damper and a plastic element in parallel:
...
\item two viscous dampers and a plastic element arranged as follows:
\begin{center}
\input{tikz_vvp}
\end{center}
