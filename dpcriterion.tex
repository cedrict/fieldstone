...
\]
so that equating both expressions of $\tau_m$ for the DP and MC criteria leads to:
\begin{eqnarray}
...
For $\theta=\pi/6$, the DP yield surface circumscribes the MC yield surface and Eq.~(\ref{eq:Fdp}) writes:
\begin{eqnarray}
F^{\text{\tiny DP}}
&=& \sqrt{{\cal I}_2({\bm \tau})} 
...
For $\theta=-\pi/6$, the DP yield surface inscribes the MC yield surface and Eq.~(\ref{eq:Fdp}) writes:
\begin{eqnarray}
F^{\text{\tiny DP}}
&=& \sqrt{{\cal I}_2({\bm \tau})} 
- \left[ -\frac{3 \sin\phi}{ 3 \sqrt{3}/2 + \sqrt{3}/2 \sin\phi}  \frac{{\cal I}_1({\bm \sigma})}{3}
...
Note that another DP formulation is to be found on the wikipedia page of the Drucker-Prager yield criterion:
\begin{eqnarray}
F^{\text{\tiny DP}}
&=& \sqrt{{\cal I}_2({\bm \tau})} 
