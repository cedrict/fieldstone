...
local_to_global=np.zeros((ndof_el,nel),dtype=np.int32)
...
for iel in range(0,nel):
    ...
    for k in range(0,m_P):
            local_to_global[ndof_V_el+k,iel]=icon_P[k,iel]+Nfem_V
             
print("compute local_to_global_V: %.3f s" % (clock.time() - start))

###############################################################################
# fill I,J arrays
# more memory intensive than f181 since we also allow the 2,2 block of 
# the stokes matrix to be added
###############################################################################
start = clock.time()
...
if assembly==2:
   ...
   counter=0
   for iel in range(0,nel):
       for i in range(0,ndof_el):
           idof=local_to_global[i,iel]
           for j in range(0,ndof_el):
               jdof=local_to_global[j,iel]
               II_V[counter]=idof
               JJ_V[counter]=jdof
               counter+=1
...
for iel in range(0,nel):
    ...
    for iq in range(0,nq_per_dim):
        for jq in range(0,nq_per_dim):
            ...
            for i in range(0,m_V):
                b_el[ndof_V*i+1]-=N_V[i]*rho[iel]*gy*JxWq
    ...
    A_el[0:ndof_V_el,0:ndof_V_el]=K_el
    A_el[ndof_V_el:ndof_el,0:ndof_V_el]=G_el.T
    A_el[0:ndof_V_el,ndof_V_el:ndof_el]=G_el
