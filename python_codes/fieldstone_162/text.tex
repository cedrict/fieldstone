...
According to \textcite{bobf08} (p.90) one can make the ${\Q}_1 \times P_0$ stable by adding {\it one}
...
Note that in this form the basis functions are not a partition of unity: if 
we write the basis functions as follows
\begin{eqnarray}
...
The weak forms of the Stokes equations are given by
\begin{eqnarray}
...
In the case of the ${\Q}_1^\star \times P_0$ element we can write the
...
for iel in range(0,nel):
    ...
    for iq in range(0,nq_per_dim_K):
        for jq in range(0,nq_per_dim_K):
            ...
            f_el[8]+=Nu[4]*bx(xq,yq)*JxWq+Nv[4]*by(xq,yq)*JxWq
\end{lstlisting}
...
\]
In the case of the ${\Q}_1^\star \times P_0$ element there is only one pressure dof per element 
...
There are in total $Nfem_V=nn_V*ndof_V+nel$ velocity degrees of freedom and $Nfem_P=nel$
