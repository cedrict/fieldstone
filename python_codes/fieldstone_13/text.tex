...
Often in colloquial language "average" refers to the arithmetic mean: \index{stones}{arithmetic mean}
\begin{equation}
\langle \phi \rangle_{am}=\frac{1}{n} \sum_k^n \phi_i 
\end{equation}
where $<\phi>_{am}$ is the arithmetic average of the $n$ numbers $\phi_i$. 
However, in mathematics other means are commonly used, such as the geometric mean: \index{stones}{geometric mean}
\begin{equation}
\langle \phi \rangle_{gm}=\left( \prod_i^n \phi_i \right)
\end{equation}
PROBLEM with this formula!!!!
and the harmonic mean: \index{stones}{harmonic mean}
\begin{equation}
