...
\begin{flushright} {\tiny {\color{gray} python\_codes/fieldstone\_109/text.tex}} \end{flushright}

\par\noindent\rule{\textwidth}{0.4pt}

\begin{center}
\inpython
{\small Code: \url{https://github.com/cedrict/fieldstone/tree/master/python_codes/fieldstone_109}}
\end{center}
...
The obstacle is centered on $(L_x/2,0)$ and has a radius of $a=200~\si{\km}$ (because of 
the symmetry of the problem I only model half of the domain).
It is assigned a viscosity $\eta=2\cdot 10^{21}~\si{\pascal\second}$ which is 1000 times 
larger than the viscosity of the channel $\eta=2\cdot 10^{18}~\si{\pascal\second}$.
...
Given the boundary conditions there is a pressure nullspace which is removed by enforcing 
that pressure is volume normalised, i.e. $\int_\Omega p \; dV=0$ (as a postprocessing
step). The element used is the Taylor-Hood pair $Q_2\times Q_1$ (see Section~\ref{MMM-ss:pairq2q1}).
...
Because of the direct solver the resolution is rather limited (see resolution tests here after). 
...
The recovered pressure is rather similar to the analytical one. Indeed we find that 
an increase in resolution and an increase in domain size (especially $L_z$) brings 
the computed pressure closer and closer to the analytical one. 
At resolution used here pressures are off by less than 10\%.
...
Velocity and pressure are exported on a line given by $y=L_y/2$ and $z=L_z/2$. 

\begin{center}
\includegraphics[width=8.5cm]{python_codes/fieldstone_109/results/line/u}
\includegraphics[width=8.5cm]{python_codes/fieldstone_109/results/line/v}\\
\includegraphics[width=8.5cm]{python_codes/fieldstone_109/results/line/w}
\includegraphics[width=8.5cm]{python_codes/fieldstone_109/results/line/p}
\end{center}

Note that the $40\times 20\times 5$ model takes approximately
240s (4min) to build the matrix and 133s to solve the linear system 
(about 5 GB of RAM are needed then). 
The $40\times 20\times 8$ model takes approximately
390s (6+min) to build the matrix and 440s to solve the linear system. 
