...
print("q1 (m,M) %.4f %.4f " %(np.min(q1),np.max(q1)))

print("compute q1 (%.3fs)" % (time.time() - start))
...
q2=np.zeros(nnp,dtype=np.float64)  
a_mat = np.zeros((nnp,nnp),dtype=np.float64)  
rhs   = np.zeros(nnp,dtype=np.float64)        
...
q2 = sps.linalg.spsolve(sps.csr_matrix(a_mat),rhs)

np.savetxt('q2.ascii',np.array([x,y,q2]).T,header='# x,y,q2')

print("q2 (m,M) %.4f %.4f " %(np.min(q2),np.max(q2)))

print("compute q2 (%.3fs)" % (time.time() - start))
...
for iel in range(0,nel):
    ...
    for iq in [-1, 1]:
        for jq in [-1, 1]:
            ...
            avrg_q1+=q1_q*wq*jcob            
            avrg_q2+=q2_q*wq*jcob            


print ("avrg(p )= %.9f" % avrg_p)
print ("avrg(q1)= %.9f" % avrg_q1)
print ("avrg(q2)= %.9f" % avrg_q2)
...
error_q1 = np.empty(nnp,dtype=np.float64)
error_q2 = np.empty(nnp,dtype=np.float64)
...
for iel in range (0,nel):
    for iq in [-1,1]:
        for jq in [-1,1]:
            ...
            errq1+=(q1_q-pressure(xq,yq))**2*wq*jcob
            errq2+=(q2_q-pressure(xq,yq))**2*wq*jcob
...
print("compute discr. errors (%.3fs)" % (time.time() - start))
...
q1_temp=np.reshape(q1,(nny,nnx))
q2_temp=np.reshape(q2,(nny,nnx))
...
error_q1_temp=np.reshape(error_q1,(nny,nnx))
error_q2_temp=np.reshape(error_q2,(nny,nnx))
...
im = axes[1][0].imshow(error_u_temp,extent=uextent,cmap='Spectral_r',interpolation='nearest')
axes[1][0].set_title('$v_x-t^{th}_x$',fontsize=10,y=1.01)
axes[1][0].set_xlabel('x')
axes[1][0].set_ylabel('y')
fig.colorbar(im,ax=axes[1][0])

im = axes[1][1].imshow(error_v_temp,extent=uextent,cmap='Spectral_r',interpolation='nearest')
axes[1][1].set_title('$v_y-t^{th}_y$',fontsize=10,y=1.01)
axes[1][1].set_xlabel('x')
axes[1][1].set_ylabel('y')
fig.colorbar(im,ax=axes[1][1])

im = axes[1][2].imshow(error_p_temp, extent=uextent, cmap='RdGy_r',interpolation='nearest')
axes[1][2].set_title('$p-p^{th}$',fontsize=10,y=1.01)
axes[1][2].set_xlabel('x')
axes[1][2].set_ylabel('y')
fig.colorbar(im,ax=axes[1][2])

im = axes[2][0].imshow(q1_temp,extent=uextent,cmap='Spectral_r',interpolation='nearest')
axes[2][0].set_title('$q_1$',fontsize=10,y=1.01)
axes[2][0].set_xlabel('x')
axes[2][0].set_ylabel('y')
fig.colorbar(im,ax=axes[2][0])

im = axes[2][1].imshow(q2_temp,extent=uextent,cmap='Spectral_r',interpolation='nearest')
axes[2][1].set_title('$q_2$',fontsize=10,y=1.01)
axes[2][1].set_xlabel('x')
axes[2][1].set_ylabel('y')
fig.colorbar(im,ax=axes[2][1])

im = axes[3][0].imshow(error_q1_temp,extent=uextent,cmap='Spectral_r',interpolation='nearest')
axes[3][0].set_title('$q_1-p^{th}$',fontsize=10,y=1.01)
axes[3][0].set_xlabel('x')
axes[3][0].set_ylabel('y')
fig.colorbar(im,ax=axes[3][0])

im = axes[3][1].imshow(error_q2_temp,extent=uextent,cmap='Spectral_r',interpolation='nearest')
axes[3][1].set_title('$q_2-p^{th}$',fontsize=10,y=1.01)
