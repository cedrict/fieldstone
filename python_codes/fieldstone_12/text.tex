...
Last revision: January 21st, 2026.
...
We have seen in \stone~\ref{f01} how to recover the elemental pressure as a postprocessing step, i.e.
by computing the velocity divergence in the middle of each element and multiplying the result by the 
penalty parameter. 
...
Remark: in general, when a discontinuous elemental pressure is used in a \stone, 
it is called $p$ while its projection onto the velocity nodes is called $q$. 
In this case we compute several nodal pressures as explained in Section~\ref{MMM-psmoothing} of {\tt manual.pdf}:
...
\item $q_5$: is not computed because (it turned out to be same as 6).

\item $q_6$: consistent pressure recovery through FE (scheme 6)
...
there is a pressure nullspace and it is removed by normalizing the pressure that $\int_\Omega p\; dV =0 $.

\begin{remark}
Nodes on edges and corners may need special treatment as documented in \textcite{sagl81a} (1981) or
\textcite{legs79} (1979) which is not done here.  
Nodes on the boundary showcase the largest errors. Maybe then use CBF instead for these?
\end{remark}

\begin{remark}
\textcite{legs79} (1979) discuss two different ways to compute the lumped mass matrix. 
\end{remark}
...
\end{eqnarray}
Both these sums are zero because on one row or one column the sum of the $\phi$ function is zero\footnote{Again, 
assuming an even number of elements in both directions!}. 
In the end we find
\begin{eqnarray}
...
\newpage
%----------------------------------------------------------------------------------------
\subsection*{Donea \& Huerta manufactured solution: Regular mesh made of square elements}
...
{\captionfont fields $p$ and $q_{1-8}$ as a function of $x$. Right panel is a zoom around $x=0$}
\end{center}
...
{\captionfont Absolute errore with regards to the analytical solution. All on $32\times 32$ mesh.}
\end{center}
...
Looking at the min/max/avg of the elemental pressure field with and without filter:
\begin{center}
...
\newpage
%............................................
\subsection*{Donea \& Huerta manufactured solution: Adding randomness to internal node positions} 
We now add a random value $\xi h$ to the location of all nodes which are not on the boundary 
where $h=L_x/nelx$ and we set $\xi=20\%$.
