...
def compute_gamma(Tq,yq):
    ydiff=yq-ytr-clapeyron_slope*(Tq-Ttr)
    if ydiff<-half_width/2.:
       gamma=0.
    elif ydiff<half_width/2.:
       gamma=0.5*(1+np.sin(np.pi/half_width*ydiff))
    else:
       gamma=1.
    #print (xq,yq,gamma,ydiff)
    return gamma
...
for istep in range(0,nstep):
    ...
    for iel in range(0,nel):
        ...
        for iq in range(0,nqperdim):
            for jq in range(0,nqperdim):
                ...
                if use_pt:
                   gamma=compute_gamma(Tq,yq)
                else:
                   gamma=0.

                # compute elemental rhs vector
                for i in range(0,mV):
                    f_el[ndofV*i+1]+=NNNV[i]*jcob*weightq*(Ra*Tq-Rb*gamma)
                #end for
    ...
    dt=CFL*(Lx/nelx)/np.max(np.sqrt(u**2+v**2))

    time+=dt

    print('dt = %.6f' %dt)

    dt_file.write("%10e %10e\n" % (time,dt))
    dt_file.flush()
    ...
    for iel in range(0,nel):
        for i in range(0,mV):
            ...
            for k in range(0,mV):
                ...
                q_x -= dNNNVdx[k]*T[iconV[k,iel]]
                q_y -= dNNNVdy[k]*T[iconV[k,iel]]
            #end for
            inode=iconV[i,iel]
            exx_n[inode]+=e_xx
            eyy_n[inode]+=e_yy
    ...
    if istep%10==0:
       ...
       vtufile.write("</DataArray>\n")
       #--
       vtufile.write("<DataArray type='Float32' Name='Gamma' Format='ascii'> \n")
       for i in range(0,NV):
           gamma=compute_gamma(T[i],yV[i])
           vtufile.write("%10e \n" %gamma)
       vtufile.write("</DataArray>\n")
       #--
