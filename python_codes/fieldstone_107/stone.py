...
for istep in range(0,nstep):
    ...
    for iel in range (0,nel):
        ...
        for iq in range(0,nqperdim):
            for jq in range(0,nqperdim):
                ...
                a_el+=B_mat.T.dot(B_mat)*weightq*jcob*K_s/eta_f
                b_el+=(dNNNdx*gx+dNNNdy*gy)*K_s/eta_f*rhoq*weightq*jcob
    ...
    u=np.zeros(N,dtype=np.float64)   # x-component velocity
    v=np.zeros(N,dtype=np.float64)   # y-component velocity

    u[:]=-K_s/eta_f*(dpdx_nodal[:]-rho[:]*gx)
    v[:]=-K_s/eta_f*(dpdy_nodal[:]-rho[:]*gy)

    ustats_file.write("%6e %6e %6e\n" % (time,np.min(u),np.max(u))) ; ustats_file.flush()
    vstats_file.write("%6e %6e %6e\n" % (time,np.min(v),np.max(v))) ; vstats_file.flush()
