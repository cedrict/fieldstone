...
\begin{flushright} {\tiny {\color{gray} python\_codes/fieldstone\_110/text.tex}} \end{flushright}

\par\noindent\rule{\textwidth}{0.4pt}

\begin{center}
\inpython
{\small Code: \url{https://github.com/cedrict/fieldstone/tree/master/python_codes/fieldstone_110}}
\end{center}

\par\noindent\rule{\textwidth}{0.4pt}

Last revision: March 15th, 2026.

\par\noindent\rule{\textwidth}{0.4pt}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The setup is taken from \textcite{blbc89} (1989) (See \stone 3, and Section~\ref{ss:blbc89}). 
The equations that we are solving are presented in Section~\ref{MMM-ss:dimeqs2}. 
...
then the $\partial T/\partial t$ term in the energy equation is zeroed and Picard iterations
are used (with a relaxation parameter of 0.25) to arrive at the steady state.
...
The shear heating term is $\Phi=2 \eta_0 \dot{\bm \varepsilon}^d:\dot{\bm \varepsilon}^d$. 
With regards to solving the energy equation this term is trivial to implement 
since it does not depend on temperature at all and it ends up in the right hand side of
the energy equation.
...
Note that because it is the gradient and not the pressure itself that enters this term then 
...
As explained in Section~\ref{MMM-ss:dimeqs2}, the above choice of parameters yields the following 
...
At low resolutions it is faster (and possible) to build the $\K$ and $\G$
blocks as full matrices and assemble them into a global array for the Stokes 
matrix too. At large(r) resolutions a sparse storage is necessary. 

\begin{lstlisting}
if sparse:
   A_sparse = lil_matrix((Nfem,Nfem),dtype=np.float64)
else:   
   K_mat = np.zeros((Nfem_V,Nfem_V),dtype=np.float64) 
...
{\color{red} results for BA are obtained with the new clean version of 
the code. Results for EBA are not. All should be rerun systematically 
on SHREK. BSc thesis? 'results\_XX' old code. RESULTS\_XX: new code.}
...
\includegraphics[width=5.7cm]{python_codes/fieldstone_110/RESULTS_BA/conv_Ra1e4.pdf}
\includegraphics[width=5.7cm]{python_codes/fieldstone_110/RESULTS_BA/conv_Ra1e5.pdf}
\includegraphics[width=5.7cm]{python_codes/fieldstone_110/RESULTS_BA/conv_Ra1e6.pdf}\\
