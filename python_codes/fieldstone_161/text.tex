...
\newpage
Let us now turn to a level 3 grid, i.e. 4x4 elements. 
...
x---x---x---x---x---x---x---x---x
\end{verbatim}
Only 21 $u$ are left, and we will find as many $v$ dofs, so in this case dim($V_h$)=42,
in accordance with the table. 

Let us now turn to the pressure dofs. 
Warning: it is not as simple as the velocity dofs. 
In \textcite{huzh11} we read: ``Meanwhile, we decrease the space $Q_1^{dc}$ 
...
it is useful to look at this part of \cite{huzh11}:
...
in 2x2 macro-elements we see that there are 3 type of vertices: the corner ones, 
the mid-edge ones, and the interior ones. 
Let us start with the simplest case: a 2x2 element mesh, i.e. a single macro element.
If the pressure field was a 'true' $Q_{-1}$ field, we would have four pdofs per element,
so 4x4=16 pdofs in total:
\begin{verbatim}
+-------+-------+
|4     3|4     3|
|       |       |
...
+-------+-------+
\end{verbatim}
However it is clear from Eq.~(3.7) that the corner nodes are constrained to 0, so we
...
We find in \textcite{zhmu16} a rewording of this above:
\begin{center}
\fbox{\includegraphics[width=12cm]{python_codes/fieldstone_161/images/zhmu16}}
\end{center}

Let us look at a 4x4 element mesh, i.e. a 2x2 macro element as in the figure above. 
\begin{verbatim}
C-------+-------+-------+-------C
|4     3|4     3|4     3|4     3|
|       |       |       |       |
...
C-------+-------+-------+-------C
\end{verbatim}
Once again we start from potentially 16x4=64 pdofs. We find that 
each vertex is subjected to a constraint, either because it on an edge betwen
or in the middle of four elements. 
...
pdofs. In thi case, 5x5=25 constraints. Taking into account the zero-average
...
Q: what are the consequences of eq(3.7) (i.e. p=0 at corners?!) for 
generic cases?

Q: what if odd number of elts? in practice, this makes no difference in the 
error values and rates.
