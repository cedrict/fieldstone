...
if int(len(sys.argv) == 15) or int(len(sys.argv) ==16): 
    R_blob       = float(sys.argv[1])
    z_blob       = float(sys.argv[2])
    rho_blob     = float(sys.argv[3])
    eta_blob     = float(sys.argv[4])
    eccentricity = float(sys.argv[5])
    path         =       sys.argv[6]
   ...
   eta_blob=1e22
   eccentricity=0
   path='solutions/trial/' #'solution/'
   hhh=50e3                # element size at the surface
   radial_model="4layer"           #'4layer'
...
if radial_model=='gravbench': 
   ...
   a_blob=300e3            #radius of blob
   b_blob=300e3            #radius of blob
   z_blob=R_outer-1000e3   #starting depth
   eta0=1e22
   eta_blob=eta0
...
if radial_model=='4layer': # 4layer model 
   ...
   if int(len(sys.argv) == 16): 
      ...
      eta_lith  =sys.argv[13] #1e21
      rho_mantle=sys.argv[14] #3500 #knaymeyer 2021
      eta_mantle=sys.argv[15] #6e20
      eta_layer=sys.argv[15]
#---------------------------------
...
# gravity_method: 2 -> 6 or 12 point quad

gravity_method=2
np_grav=80             #adjusted angle to run full halfsphere and added extra measurement points
...
###############################################################################
# 12 point integration coeffs and weights 
###############################################################################
...
if np_blob>0:
   ...
   pts_bl = np.stack([a_blob*np.cos(theta_bl),z_blob+b_blob*np.sin(theta_bl)], axis=1) #points on blob outersurface 
...
np.savetxt(path+'surface_Pnode.ascii',np.array([xP,zP,surface_Pnode,rP]).T,header='# x,y,flag,r')
np.savetxt(path+'cmb_Pnode.ascii',np.array([xP,zP,cmb_Pnode,rP]).T,header='# x,y,flag,r')
...
for i in range(0,NV):
    eta_nodal[i],rho_nodal[i]=material_model(xV[i],zV[i],eta_blob,rho_blob,z_blob,a_blob,b_blob,npt_rho,\
...
for iel in range(0,nel):
    eta_eltal[iel],rho_eltal[iel]=material_model(xc[iel],zc[iel],eta_blob,rho_blob,z_blob,a_blob,b_blob,npt_rho,\
                                                 npt_eta,profile_rho,profile_eta,blobtype)
...
for istep in range(0,nstep):
    ...
    if solve_stokes:
        ...
        for iel in range(0,nel):
            ...
            for kq in range (0,nqel):
                ...
                inertia+=jcob*weightq*rhoq * 2*np.pi*xq**2 #NOT SURE
    ...
    tracfile=open(path+'surface_traction_nodal_{:04d}.ascii'.format(istep),"w")
    ...
    tracfile=open(path+'surface_vr_{:04d}.ascii'.format(istep),"w")
    for i in range(0,NV):
        if surface_node[i]: 
    ...
    tracfile=open(path+'surface_vt_{:04d}.ascii'.format(istep),"w")
    for i in range(0,NV):
        if surface_node[i]: 
           tracfile.write("%10e %10e \n" \
    ...
    tracfile=open(path+'cmb_traction_nodal_{:04d}.ascii'.format(istep),"w")
    for i in range(0,NV):
        if cmb_node[i]: 
    ...
    tracfile=open(path+'cmb_vr_{:04d}.ascii'.format(istep),"w")
    for i in range(0,NV):
        if cmb_node[i]: 
           tracfile.write("%10e %10e \n" \
                          %(theta_nodal[i],u[i]*np.sin(theta_nodal[i])+v[i]*np.cos(theta_nodal[i])  ))
    ...
    tracfile=open(path+'cmb_vt_{:04d}.ascii'.format(istep),"w")
    for i in range(0,NV):
        if cmb_node[i]: 
           tracfile.write("%10e %10e \n" \
                          %(theta_nodal[i],u[i]*np.cos(theta_nodal[i])-v[i]*np.sin(theta_nodal[i]) ) )
    ...
    #Dsurf_rho = []
    dynfile=open(path+'dynamic_topography_surf_{:04d}.ascii'.format(istep),"w")
    for i in range(0,NV):
        if surface_node[i]:
           dyntop_i= -(tau_rr_nodal[i]-q[i])/((rho_nodal[i]-density_above)*g0) 
           dynfile.write("%e %e\n" %(theta_nodal[i],dyntop_i))
           #Dsurf_rho = np.append(Dsurf_rho,[rho_nodal[i]-density_above])
    dynfile.close()
    
    #Dcmb_rho = []
    dynfile=open(path+'dynamic_topography_cmb_{:04d}.ascii'.format(istep),"w")
    for i in range(0,NV):
        if cmb_node[i]:
           dyntop_i= -(tau_rr_nodal[i]-q[i])/((density_below - rho_nodal[i])*g0) 
           dyntop_i_aspect= -(tau_rr_nodal[i]-q[i]+avrg_p_cmb)/((density_below - rho_nodal[i])*g0) 
           dynfile.write("%e %e %e \n" %(theta_nodal[i],dyntop_i,dyntop_i_aspect))
           #Dcmb_rho = np.append(Dcmb_rho,[density_below - rho_nodal[i]])
    dynfile.close()
    
    #avr_surf_drho = sum(Dsurf_rho)/len(Dsurf_rho)
    #avr_cmb_drho = sum(Dcmb_rho) /len(Dcmb_rho)


    dynfile=open(path+'model_aspects_{:04d}.ascii'.format(istep),"w")
    dynfile.write("# Mass(kg) Ggrav(m3/kgs2) Planetairy Radius(m) Delta_rho_surface Delta_rho_CMB rho_mantle rho_lith Inertia ProfMass ProfIner  \n")
    dynfile.write("%e %e %e %e %e %e %e %e %e %e \n" %(mass, Ggrav,R_outer,rho_mantle-density_above,density_below-rho_mantle,rho_mantle ,rho_lith, inertia, Prof_masse,Prof_inertia))
    #dynfile.write("%e %e %e %e %e %e %e %e \n" %(mass, Ggrav,R_outer,avr_surf_drho,avr_cmb_drho,inertia, Prof_masse,Prof_inertia))
    dynfile.close()
    ...
    filename = path+'solution_{:04d}.vtu'.format(istep)
    ...
    if radial_model=='gravbench': 
       testfile=open(path+'gravity_benchmark.ascii',"w")
       testfile.write("# angle gr_meas gr_anal \n")

    dphi=2*np.pi/nel_phi
    for i in range(0,np_grav):
        angleM[i]=np.pi/2-np.pi/(np_grav-1)*i    #adjusted angleM to run full halfsphere 
        ...
        if gravity_method==0:
           ...
           gvect_x[i],gvect_y[i],gvect_z[i]=compute_gravity_at_point1(xM[i],yM[i],zM[i],nel,xV,zV,iconV,arear,dphi,nel_phi,\
                                                                      eta_blob,rho_blob,z_blob,a_blob,b_blob,npt_rho,\
                                                                      npt_eta,profile_rho,profile_eta,blobtype)
        ...
        if radial_model=='gravbench' and test==2:
           ...
           gr=Ggrav*rho_blob*VOL_blob/dist2
    ...
    gravfile1 = path+'gravity_{:04d}.ascii'.format(istep)
    ...
    gravfile2 = path+'gravityanomaly_wp_{:04d}.ascii'.format(istep)
    ...
    gravfile3 = path+'gravityanomaly_np_{:04d}.ascii'.format(istep)
    ...
    if istep>0:
       gravfile4 = path+'gravity_diff_{:04d}.ascii'.format(istep)
       np.savetxt(gravfile4,np.array([xM,yM,zM,rM,angleM,gvect_x-gvect_x_0,gvect_y-gvect_y_0,gvect_z-gvect_z_0,gvect-gvect_0]).T,fmt='%.6e')
    else:
       gvect_x_0[:]=gvect_x[:]
       gvect_y_0[:]=gvect_y[:]
       gvect_z_0[:]=gvect_z[:]
