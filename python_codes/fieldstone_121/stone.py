    ...
    for iel in range(0,nel):
        ...
        for iq in range(0,nqperdim):
            for jq in range(0,nqperdim):
                ...
                K_el+=b_mat.T.dot(c_mat.dot(b_mat))*viscosity(xq,yq,ee,Tq)*weightq*jcob
...
ee  = np.zeros(nel,dtype=np.float64)  
...
for iel in range(0,nel):
    ...
    ee[iel]=np.sqrt(0.5*(exx[iel]*exx[iel]+eyy[iel]*eyy[iel])+exy[iel]*exy[iel])
...
for iel in range (0,nel):
    vtufile.write("%10e\n" % viscosity(xc[iel],yc[iel],ee[iel],Tc[iel]))
vtufile.write("</DataArray>\n")
#--
