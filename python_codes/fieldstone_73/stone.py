...
bc_inds = np.sort(np.hstack((raw_b_inds*ndof_V, raw_b_inds*ndof_V+1)))
...
B=np.array([[dNdxyeq[0], meq_null],
            [meq_null,   dNdxyeq[1]],
            [dNdxyeq[1], dNdxyeq[0]]]).reshape(3, ndof_V*m, nel, nqel,order='F')

# compute elemental A_fem matrix
A_el=(np.einsum('jieq, jk, kleq, eq->eil', B, C, B, jcob) *viscosity *weightqq)

# compute elemental b_fem vector
...
B = np.array([[dNdxye[0], me_null],
              [me_null,   dNdxye[1]],
              [dNdxye[1], dNdxye[0]]]).reshape(3, ndof_V*m, nel, order='F')

# update elemental matrix
A_el+=(np.einsum('jie, jk, kle, e->eil', B, H, B, jcob)*penalty*weightq)

# assemble matrix A_fem and right hand side b_fem
m_indices=((ndof_V*icon).T[:,np.newaxis,:] 
         + np.indices((ndof_V,))[0,np.newaxis,:,np.newaxis]) # iel, k1, i1

mkk_indices = m_indices.reshape(nel, ndof_V*m, order='F') # iel, 1kk
mm_indices = (mkk_indices[:, :, np.newaxis] + 0*mkk_indices[:, np.newaxis, :]) # iel, 1kk, 2kk
mm_indices = (mm_indices, np.einsum('ijm -> imj', mm_indices))

np.add.at(b_fem, m_indices, b_el)
np.add.at(A_fem, mm_indices, A_el)

print("Build FE matrix: %.4f s | %d" % (clock.time()-start,Nfem))
...
b_fem -= np.einsum('ij, i', A_fem[bc_inds, :], bc_vals)
bc_diag = A_fem[bc_inds, bc_inds]  # this returns a new array, not a view.
b_fem[bc_inds] = bc_diag * bc_vals

A_fem[bc_inds, :] = 0.
A_fem[:, bc_inds] = 0.
A_fem[bc_inds, bc_inds] = bc_diag

# print("A_fem (m,M) = %.4f %.4f" %(np.min(A_fem), np.max(A_fem)))
# print("b_fem   (m,M) = %.6f %.6f" %(np.min(b_fem), np.max(b_fem)))

print("Impose b.c.: %.4f s | %d" % (clock.time()-start,Nfem))
...
sol=spsolve(csr_matrix(A_fem),b_fem)

print("Solve time: %.4f s | %d" % (clock.time()-start,Nfem))
...
if debug:
   np.savetxt('pressure.ascii', np.array([xyc[:, 0], xyc[:, 1], p]).T,
              header='# xc,yc,p')
   np.savetxt('strainrate.ascii', np.array([xyc[:, 0], xyc[:, 1],exx, eyy, exy]).T,
...
pq = np.einsum('i, j->ij', p, np.ones(nqel))
...
print("compute errors: %.5f s | %d" % (clock.time()-start,Nfem))
