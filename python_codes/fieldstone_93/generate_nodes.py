...
nodesfile.write("%5d %5d %3d %3d\n" %(np_left+np_right+np_top+np_bottom+np_sphere+np_surf+1,2,0,1))
...
for i in range (0,np_sphere):
    angle=2*np.pi/(np_sphere-1)*i
    x=0.5+rad*np.cos(angle)
    y=0.6+rad*np.sin(angle)
    nodesfile.write("%5d %10e %10e %3d\n" %(counter+1,x,y,0))
    counter+=1
    if i<np_sphere-1:
       counter_segment+=1
