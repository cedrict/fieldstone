...
e1   =np.zeros(NV,dtype=np.float64)  
...
for iter in range(0,niter):
    ...
    NNNV  = np.zeros(m,dtype=np.float64)             # shape functions
    ...
    for iel in range(0, nel):
        ...
        for iq in [-1, 1]:
            for jq in [-1, 1]:
                ...
                for i in range(0, m):
                    f_el[ndofV*i  ]+=NNNV[i]*jcob*wq*rhoq*gx
                    f_el[ndofV*i+1]+=NNNV[i]*jcob*wq*rhoq*gy
