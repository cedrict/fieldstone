...
Note that $H=0$ in the experiments carried out in what follows.
...
with
\begin{align}
{\M} &= \int_\Omega \varphi \beta \vec{\bN}^T \vec{\bN} dV\nn \\
\K_d &= \int_\Omega {\bm B}^T {\bm B} \frac{K}{\eta_f} dV \nn \\
\vec{b} &= \int_\Omega \vec\bN^T  H dV \nn
\end{align}
...
for iel in range (0,nel):
    ...
    for iq in [-1,1]:
        for jq in [-1,1]:
            ...
            A_el+=MM+Kd*dt
            b_el+=MM.dot(pvect)+N_P[:]*JxWq*(H[iel]*dt-
\end{lstlisting}
...
The transient evolution of the excess pore pressure is governed by Eq.~\eqref{f128:f128:eq1}
and is subject to the boundary conditions as specified in the following figure:
\begin{center}
...
mesh is stored in a {\tt .mat} format which is very inconvenient. In this \stone I rely on 
linear quadrilaterals ($Q_1$) instead. 

In what follows a $500\times 200$ elements mesh is used. Because of the geometry of the fault 
and the fact that we rely on a regular mesh, the fault is then represented as follows in the model:

\begin{center}
\includegraphics[width=7cm]{python_codes/fieldstone_128/RESULTS/experiment2/K}
\includegraphics[width=6.5cm]{python_codes/fieldstone_128/RESULTS/experiment2/K_zoom}\\
...
obtained by normalising the velocity vector to one in each element. This is easily done in ParaView
by not scaling the arrow glyphs with any field.}
\end{center}
...
%--------------------------------------------------------------------------------------
\section*{Fluid Flow in a random medium ({\tt experiment=3})}
...
\begin{center}
\includegraphics[width=10cm]{python_codes/fieldstone_128/RESULTS/experiment3/cells}
\end{center}
