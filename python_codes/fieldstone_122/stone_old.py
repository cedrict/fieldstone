...
eps1=0.5
eps2=1.5
weakening_factor_phi=0.5
weakening_factor_cohesion=0.5

A_values=[398.1,6.80220477]
Q_values=[356e3,534e3]
n_values=[3,5.52]
...
#################################################################
# define boundary conditions for Stokes
#################################################################
start = time.time()
...
for i in range(0,NV):
    #left boundary 
    if xV[i]<eps:
       #bc_fixV[i*ndofV+0] = True ; bc_val[i*ndofV  ] = yV[i]*(Ly-yV[i])*1e-5
       bc_fixV[i*ndofV+1] = True ; bc_val[i*ndofV+1] = 0 # vy
    #right boundary 
    if xV[i]/Lx>1-eps:
       #bc_fixV[i*ndofV+0] = True ; bc_val[i*ndofV  ] = yV[i]*(Ly-yV[i])*1e-5
       bc_fixV[i*ndofV+1] = True ; bc_val[i*ndofV+1] = 0 # vy
    #bottom boundary 
    if yV[i]/Ly<eps:
       bc_fixV[i*ndofV+0] = True ; bc_val[i*ndofV  ] = -v0 # vx
       bc_fixV[i*ndofV+1] = True ; bc_val[i*ndofV+1] = 0   # vy
    #top boundary 
    if yV[i]/Ly>1-eps:
       bc_fixV[i*ndofV+0] = True ; bc_val[i*ndofV  ] = +v0 # vx
...
#################################################################
# define boundary conditions for pore fluid pressure
#################################################################
start = time.time()

bc_fixPf=np.zeros(NfemV,dtype=np.bool)  # boundary condition, yes/no
bc_valPf=np.zeros(NfemV,dtype=np.float64)  # boundary condition, value

#for i in range(0,NV):
#    bc_fixPf[i] = True ; bc_valPf[i] = 0 
#end for

print("setup: boundary conditions: %.3f s" % (time.time() - start))

#################################################################
# define initial pore fluid pressure
#################################################################
start = time.time()

Pf     =np.zeros(NV,dtype=np.float64)  
Pf_prev=np.zeros(NV,dtype=np.float64)  
...
print("setup: initial Pf: %.3f s" % (time.time() - start))


#################################################################
# define initial porosity phi_m
#################################################################
start = time.time()

phi_m =np.zeros(NV,dtype=np.float64)  
...
swarm_total_strainxx=np.empty(nmarker,dtype=np.float64)     # strain xx
swarm_total_strainxy=np.empty(nmarker,dtype=np.float64)     # strain yy
swarm_total_strainyy=np.empty(nmarker,dtype=np.float64)     # strain xy
swarm_total_strain_eff=np.empty(nmarker,dtype=np.float64)   # effective strain 
swarm_plastic_strainxx=np.empty(nmarker,dtype=np.float64)   # plastic strain xx
swarm_plastic_strainxy=np.empty(nmarker,dtype=np.float64)   # plastic strain yy
swarm_plastic_strainyy=np.empty(nmarker,dtype=np.float64)   # plastic strain xy
swarm_plastic_strain_eff=np.empty(nmarker,dtype=np.float64) # effective plastic strain xx
...
swarm_tau_eff=np.empty(nmarker,dtype=np.float64)            # effective dev stress
...
swarm_sw_level=np.empty(nmarker,dtype=np.float64)           # level of strain weaking in [0,1] 
swarm_tau_angle=np.empty(nmarker,dtype=np.float64)          # principal angle  
swarm_sigmaxx=np.empty(nmarker,dtype=np.float64)            # stress xx
swarm_sigmaxy=np.empty(nmarker,dtype=np.float64)            # stress yy
swarm_sigmayy=np.empty(nmarker,dtype=np.float64)            # stress xy
swarm_sigma_angle=np.empty(nmarker,dtype=np.float64)        # principal stress angle  
...
for istep in range(0,nstep):
    ...
    for iel in range (0,nel):
        ...
        b_el=(MM-(Ka+Kd)*dt*0.5).dot(Pfvect)
    ...
    print("build FE matrix for Pf: %.3f s" % (timing.time() - start))
    ...
    print("solve Pf time: %.3f s" % (timing.time() - start))
    ...
    if True:
       for im in range(0,nmarker):
           ...
           swarm_total_strain_eff[im]  =np.sqrt(0.5*(swarm_total_strainxx[im]**2+swarm_total_strainyy[im]**2+2*swarm_total_strainxy[im]**2))
           swarm_plastic_strain_eff[im]=np.sqrt(0.5*(swarm_plastic_strainxx[im]**2+swarm_plastic_strainyy[im]**2+2*swarm_plastic_strainxy[im]**2))
