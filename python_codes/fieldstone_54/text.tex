...
value of $f \tau$ is reported in the right figure
for various wavelengths and viscosity ratios $r=\eta_2/\eta_1$:

\begin{center}
%\includegraphics[width=8cm]{python_codes/fieldstone_54/RESULTS/exp10/slope.pdf}
...
It looks liek the depth of the model plays a role in this, 
but we should probably test the influence of\footnote{It would probably not hurt 
to carry out a few experiments with ASPECT with sticky air and tons of mesh refinement
along the material interfaces}:
\begin{itemize}
\item value of $w_0$
\item time step $\delta t$
\item horizontal resolution
\item vertical resolution
\item vertical extent of domain
\item proper linear fit of the amplitude decay $f \tau$
\end{itemize}

There seems to be a funny effect for $r=1$, $Ly=5000$km and large values of $\lambda$.
This needs to be investigated. A resolution effect?
