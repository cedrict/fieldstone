...
stats_oxy_file=open('stats_oxy.ascii',"w")
...
stats_C1_file=open('stats_C1.ascii',"w")
stats_C2_file=open('stats_C2.ascii',"w")
...
stats_C1pC2_file=open('stats_C1+C2.ascii',"w")
...
for istep in range(0,nstep):
    ...
    for i in range(0,NV):
        ...
        #end if
        if Z[i]<=0:
           print('i=',i,'x,y=',xV[i],yV[i],'C1=',C1[i],'C2=',C2[i],'Z1=',Z1,'Z2=',Z2)
           exit("Z<=0")
        #end if
    #end for
    ...
    print('     corresponds to CFL nb= %f ' %CFL)
    ...
    for ifield in range(0,6): # C1 C2 tauxx tauyy tauxy
        ...
        fvect      = np.zeros(mV,dtype=np.float64)   
        ...
        for iel in range (0,nel):
            ...
            a_el=MM+Ka*dt*alpha
            b_el=(MM-Ka*(1.-alpha)*dt).dot(fvect)
        ...
        print("     -> tau_supg (m,M) %.4e %.4e " %(np.min(tau_supg),np.max(tau_supg)))

        np.savetxt('tau_supg.ascii',np.array(tau_supg).T)
        ...
        if filter_compositions and ifield<2:
           if ifield==0:
              print("     C1 (m,M)  %.4f %.4f " %(np.min(field),np.max(field)))
              stats_C1_file.write("%e %e %e \n" %(time,np.min(field),np.max(field))) ; stats_C1_file.flush()
           if ifield==1:
              print("     C2 (m,M)  %.4f %.4f " %(np.min(field),np.max(field)))
        ...
        print("     -> field (m,M)  %.4f %.4f " %(np.min(field),np.max(field)))

        if ifield==0:
           C1[:]=field[:]
           print("advect C1 time: %.3f s" % (timing.time() - start))
           stats_C1_file.write("%e %e %e \n" %(time,np.min(C1),np.max(C1))) ; stats_C1_file.flush()
           np.savetxt('C1.ascii',np.array([xV,yV,C1]).T)

        if ifield==1:
           C2[:]=field[:]
           print("advect C2 time: %.3f s" % (timing.time() - start))
           stats_C2_file.write("%e %e %e \n" %(time,np.min(C2),np.max(C2))) ; stats_C2_file.flush()
           np.savetxt('C2.ascii',np.array([xV,yV,C2]).T)
