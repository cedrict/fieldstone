...
def viscosity(x,y):
    ...
    #left ridge
    if x>xD and x<(xA-hx) and y>yC and y<(yD-yA)/(xD-(xA-hx))*(x-xD)+yD:
       val=eta_rA
    if x>(xA+hx) and x<xE and y>yC and y<(yE-yA)/(xE-(xA+hx))*(x-xE)+yE:
       val=eta_rA
    if abs(x-xA)<hx and y>yC:
       val=eta_rA

    #right ridge
    if x>xF and x<(xB-hx) and y>yC and y<(yF-yB)/(xF-(xB-hx))*(x-xF)+yF:
       val=eta_rB
    if x>(xB+hx) and x<xH and y>yC and y<(yH-yB)/(xH-(xB+hx))*(x-xH)+yH:
       val=eta_rB
    if abs(x-xB)<hx and y>yC:
       val=eta_rB
...
def density(x,y):
    ...
    if x>xD and x<xA and y>yC and y<(yD-yA)/(xD-xA)*(x-xD)+yD:
       val=rho_r
    if x>xA and x<xE and y>yC and y<(yE-yA)/(xE-xA)*(x-xE)+yE:
       val=rho_r

    #right ridge
    if x>xF and x<xB and y>yC and y<(yF-yB)/(xF-xB)*(x-xF)+yF:
       val=rho_r
...
if int(len(sys.argv) == 10): 
   ...
   rB = int(sys.argv[3])
   o = int(sys.argv[4])
   v_L = float(sys.argv[5])
   v_R = float(sys.argv[6])
   nelx=int(sys.argv[7])
...
xD=xA-100*km*gamma ; yD=yC
xE=xA+100*km*gamma ; yE=yC
xF=xB-100*km*gamma ; yF=yC
xH=xB+100*km*gamma ; yH=yC
...
print('eta_rB',eta_rB)
print('eta_o',eta_o)
print('v_L',v_L/cm*year)
print('v_R',v_R/cm*year)
print('nelx',nelx)
...
vR_out=-v_R * ( Ly - 0.5*(y_in+y_out)  ) / ( 0.5*(y_in+y_out)) + v_mantle
vL_out=-v_L * ( Ly - 0.5*(y_in+y_out)  ) / ( 0.5*(y_in+y_out)) + v_mantle

if symm_bc==1:
   voutavrg=(abs(vR_out)+abs(vL_out))/2
   vR_out=-voutavrg
   vL_out=+voutavrg
