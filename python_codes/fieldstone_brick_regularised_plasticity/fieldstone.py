...
def eta_eff(exx,eyy,exy,eta_ref):
    sr=np.sqrt(0.5*(exx**2+eyy**2)+exy**2)
    eta_min=1e21
    eta_max=1e25
    sr_ref=1e-15
    nref=100
    KK=((eta_ref-eta_min)/(eta_max-eta_min))**(nref/(1.-nref))
    val=eta_min+(eta_max-eta_min)*(1+KK*(sr/sr_ref))**((1./nref-1.))
    #----vrm---------------
    #val=eta_ref*sr_ref/2/sr
    #val=min(val,eta_max)
    #val=max(val,eta_min)
    return val
...
Lx=40.e3  # horizontal extent of the domain 
Ly=10.e3  # vertical extent of the domain 
...
for iter in range(0,niter):
    ...
    for iel in range(0,nel):
        ...
        for iq in [0,1,2]:
            for jq in [0,1,2]:
                ...
                K_el+=b_mat.T.dot(c_mat.dot(b_mat))*eta_eff(exxq,eyyq,exyq,eta_ref)*weightq*jcob

                # compute elemental rhs vector
                for i in range(0,mV):
                    f_el[ndofV*i  ]+=NV[i]*jcob*weightq*rho*gx
                    f_el[ndofV*i+1]+=NV[i]*jcob*weightq*rho*gy
    ...
    for iel in range(0,nel):
        ...
        eta_el[iel]=eta_eff(exx[iel],eyy[iel],exy[iel],eta_ref)
    ...
    print("     -> eta (m,M) %.4e %.4e " %(np.min(eta_el),np.max(eta_el)))
...
np.savetxt('velocity_top.ascii',np.array([x[nnp-nnx+1:nnp],u[nnp-nnx+1:nnp],v[nnp-nnx+1:nnp]]).T,header='# x,u,v')
np.savetxt('strainrate_top.ascii',np.array([xc[nel-nelx+1:nel],e[nel-nelx+1:nel]]).T,header='# x,e')
