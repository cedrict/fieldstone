...
normalising coefficient of the stress is exactly 1 (since $h=L_x/nelx=1/64)$ so it is not implemented).
...
by means of the C$->$N algorithm or a least square algorithm (LS). 
...
on the boundary of the domain. On the top boundary, ${\bm n}=(0,1)$ so that ${\bm t}=(\sigma_{xy}, \sigma_{yy})^T$.
...
\todo{explain how Medge is arrived at!}

\todo{compare with ASPECT ??!}
