...
for j in range(0,nely):
   ...
   for i in range(0,nelx):
      x[counter]=(i+1-0.5)*sx
      y[counter]=(j+1-1)*sy
      counter+=1
   # middle line
   for i in range(0,nelx+1):
      x[counter]=(i)*sx
      y[counter]=(j+1-0.5)*sy
      counter+=1
...
np.savetxt('gridpoints.ascii',np.array([x,y]).T,header='# x,y')
...
for iel in range(0, nel):
    ...
    for iq in [-1, 1]:
        for jq in [-1, 1]:
            ...
            # midle point variant
            N[0]=0.25*(1-rq**2-2*sq+sq**2)
            N[1]=0.25*(1+2*rq+rq**2-sq**2)
            N[2]=0.25*(1-rq**2+2*sq+sq**2)
            N[3]=0.25*(1-2*rq+rq**2-sq**2)
            dNdr[0]=0.5*(-rq)   ; dNds[0]=0.5*(-1+sq)
            dNdr[1]=0.5*(1+rq)  ; dNds[1]=0.5*(-sq)
            dNdr[2]=0.5*(-rq)   ; dNds[2]=0.5*(1+sq)
            dNdr[3]=0.5*(-1+rq) ; dNds[3]=0.5*(-sq)

            # mid value variant (MV)
            N[0]=0.25*(1-2*sq-1.5*(rq**2-sq**2))
            N[1]=0.25*(1+2*rq+1.5*(rq**2-sq**2))
            N[2]=0.25*(1+2*sq-1.5*(rq**2-sq**2))
            N[3]=0.25*(1-2*rq+1.5*(rq**2-sq**2))
            dNdr[0]=-0.75*rq     ; dNds[0]=-0.5+0.75*sq
            dNdr[1]=0.5+0.75*rq  ; dNds[1]=-0.75*sq
            dNdr[2]=-0.75*rq     ; dNds[2]=0.5+0.75*sq
            dNdr[3]=-0.5+0.75*rq ; dNds[3]=-0.75*sq
            ...
            K_el+=b_mat.T.dot(c_mat.dot(b_mat))*mu(xq,yq,experiment)*wq*jcob

            # compute elemental rhs vector
            for i in range(0, m):
                f_el[ndofV*i  ]+=N[i]*jcob*wq*bx(xq,yq,experiment)
                f_el[ndofV*i+1]+=N[i]*jcob*wq*by(xq,yq,experiment)
...
for iel in range(0,nel):
    ...
    ue[iel]=(u[icon[0,iel]]+u[icon[1,iel]]+u[icon[2,iel]]+u[icon[3,iel]])/4.
    ve[iel]=(v[icon[0,iel]]+v[icon[1,iel]]+v[icon[2,iel]]+v[icon[3,iel]])/4.
    ...
    visc[iel]=mu(xc[iel],yc[iel],experiment)
...
ue_temp=np.reshape(ue,(nely,nelx))
ve_temp=np.reshape(ve,(nely,nelx))
...
onePlot(ue_temp,      0, 0, "elemental $v_x$",       "x", "y", pextent, Lx, Ly, 'Spectral_r')
onePlot(ve_temp,      0, 1, "elemental $v_y$",       "x", "y", pextent, Lx, Ly, 'Spectral_r')
...
onePlot(dens_temp,    2, 1, "b_y",                   "x", "y", uextent,  0,  0, 'RdGy_r')
onePlot(visc_temp,    2, 2, "viscosity",             "x", "y", uextent,  0,  0, 'bone_r')

axes[2][1].quiver(x,y,u,v, alpha=.95)
