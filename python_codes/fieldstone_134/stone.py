...
###############################################################################
# I have removed the first 3 columns off the .csv file downloaded 
# from the cite (in open office), and copy-pasted the result in a txt file
###############################################################################

file=open("EuroMedCentrMomTensors.txt", "r")
lines = file.readlines()
file.close
...
C_Mrt=np.zeros(C_nb,dtype=np.float64)
C_Mrp=np.zeros(C_nb,dtype=np.float64)
C_Mb=np.zeros(C_nb,dtype=np.float64)

for il in range(0,C_nb):
    vals=lines[il].strip().split()
    C_lat[il]=float(vals[0])
    C_lon[il]=float(vals[1])
    C_depth[il]=float(vals[2])
    C_Mb[il]=float(vals[3])

print('----------------------------------------')
print('latitude (m/M)',min(C_lat),max(C_lat))
print('longitude (m/M)',min(C_lon),max(C_lon))
print('depth (m/M)',min(C_depth),max(C_depth))
print('Mrr (m/M)',min(C_Mb),max(C_Mb))
#print('Mrr (m/M)',min(C_Mrr),max(C_Mrr))
#print('Mtt (m/M)',min(C_Mtt),max(C_Mtt))
#print('Mpp (m/M)',min(C_Mpp),max(C_Mpp))
#print('Mrt (m/M)',min(C_Mrt),max(C_Mrt))
...
if True:
   vtufile=open("cmt_mediterranean.vtu","w")
   vtufile.write("<VTKFile type='UnstructuredGrid' version='0.1' byte_order='BigEndian'> \n")
   vtufile.write("<UnstructuredGrid> \n")
   vtufile.write("<Piece NumberOfPoints=' %5d ' NumberOfCells=' %5d '> \n" %(C_nb,C_nb))
   #####
   ...
   vtufile.write("</DataArray>\n")
   #--
   vtufile.write("<DataArray type='Float32' Name='Mb' Format='ascii'> \n")
   for i in range(0,C_nb):
       vtufile.write("%10e \n" %(C_Mb[i]))
   vtufile.write("</DataArray>\n")
   #--
