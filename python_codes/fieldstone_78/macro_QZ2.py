...
def mesher(Lx,Ly,nelx,nely,nel,NV,mV):
    ...
    for j in range(0,nely):
        for i in range(0,nelx):
            ...
            inode9 =np3+(i+(j)*nelx)*5+0
            inode10=np3+(i+(j)*nelx)*5+1
            inode11=np3+(i+(j)*nelx)*5+2
            inode12=np3+(i+(j)*nelx)*5+3
            inode13=np3+(i+(j)*nelx)*5+4

            #print('---------------')
            #print(inode1+1,inode2+1,inode3+1,inode4+1,inode5+1,inode6+1,inode7+1,inode8+1)
            #print(inode9+1,inode10+1,inode11+1,inode12+1,inode13+1)
            ...
            iconV[2,counter]=inode9
            iconV[3,counter]=inode13
            counter=counter+1
