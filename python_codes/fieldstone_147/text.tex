...
Last revision: January 5th, 2025.
...
As far as I know there are 4 different approaches to solve the Stokes system:
...
\end{itemize}
\item Iterative methods applied to the Schur complement, with or without the $L_2$
...
but we will then need a good(enough?) preconditioner.
...
\item Transient solver methods \cite{raud22} which will not be taken into consideration here.
\end{itemize}

All these methods should also be used with a preconditioner, which we will investigate later on.

Iterative solvers require a tolerance value and it is set to $10^{-7}$ unless mentioned 
otherwise (I do carry out a sensitivity analysis later on).

Note that modern solvers presented in for example 
\textcite{geum09} (2009) or 
\textcite{krhb12} (2012) are rather complex. It is 
based on FGMRES and clever preconditioners. They will not be explored here.

It should be acknowledged that feeding the full Stokes matrix to a bunch of iterative 
solvers without thinking much about it is rather naive.


%===========================================================
\section*{The list of implemented solver strategies}

The solvers all follow the same nomenclature: {\tt OOOO-IIII-PPPP}
where {\tt OOOO} indicates the type of outer solver applied
to the full Stokes matrix (or to the Schur complement), 
{\tt IIII} indicates the type of inner solver
used on the $\K$ block (if applicable) and {\tt PPPP} indicates 
an additional option/stragegy. If one of these is absent or irrelevant it 
is indicated by an {\tt X}.

\begin{itemize}

\item \lstinline{solver=1} (Full Stokes Matrix): {\tt DIRECT-X-X}
the usual one I use in all the stones (i.e. direct solver on Stokes matrix).
\begin{lstlisting}
   sol=sps.linalg.spsolve(sparse_matrix,rhs,use_umfpack=False)
\end{lstlisting}

\item \lstinline{solver=2} (Full Stokes Matrix):  {\tt GMRES-X-X}
the gmres from linalg. 
\begin{lstlisting}
   sol,info=sps.linalg.gmres(sparse_matrix,b_fem,restart=2000,tol=tolerance)
\end{lstlisting}

\item \lstinline{solver=3} (Full Stokes Matrix): {\tt LGMRES-X-X}
 lgmres from linalg.
\begin{lstlisting}
   sol=sps.linalg.lgmres(sparse_matrix,b_fem,atol=1e-16,tol=tolerance)[0]
\end{lstlisting}

\item \lstinline{solver=4} (Schur complement approach): {\tt SCHURCG-DIRECT-X}
my Schur complement CG solver using direct solver for inner solve on $\K$ matrix.
\begin{lstlisting}
   solV,p,niter=schur_complement_cg_solver(K_mat,G_mat,M_mat,f_rhs,h_rhs,\
                                          NfemV,NfemP,niter_max,tolerance,use_precond,'direct')
\end{lstlisting}

\item \lstinline{solver=5} (Full Stokes Matrix): {\tt MINRES-X-X} 
minres from linalg
\begin{lstlisting}
   sol=sps.linalg.minres(sparse_matrix,b_fem, tol=1e-10)[0]
\end{lstlisting}

\item \lstinline{solver=6} (Full Stokes Matrix): {\tt QMR}
qmr from linalg.
\begin{lstlisting}
   sol=sps.linalg.qmr(sparse_matrix,b_fem,tol=tolerance)[0]
\end{lstlisting}

\item \lstinline{solver=7}  (Full Stokes Matrix) {\tt TFQMR}
 tfqmr from linalg
\begin{lstlisting}
   sol=sps.linalg.tfqmr(sparse_matrix,b_fem, tol=1e-10)[0]
\end{lstlisting}

\item \lstinline{solver 8}  (Full Stokes Matrix) {\tt GCROTMK}
gcrotmk from linalg
\begin{lstlisting}
   sol=sps.linalg.gcrotmk(sparse_matrix,b_fem,atol=1e-16, tol=1e-10)[0]
\end{lstlisting}

\item \lstinline{solver 9} (Full Stokes Matrix) {\tt BICG-X-X}
bicg from linalg
\begin{lstlisting}
   sol=sps.linalg.bicg(sparse_matrix,b_fem, tol=tolerance)[0] 
\end{lstlisting}

\item \lstinline{solver=10}  (Full Stokes Matrix) {\tt BICGSTAB-X-X}
 bicgstab from linalg
\begin{lstlisting}
   sol=sps.linalg.bicgstab(sparse_matrix,b_fem, tol=tolerance)[0]
\end{lstlisting}

\item\lstinline{solver=11}  (Full Stokes Matrix) {\tt DIRECT-X-UMFPACK}
direct solver spsolve using umfpack 
\begin{lstlisting}
   sol=sps.linalg.spsolve(sparse_matrix,rhs,use_umfpack=True)
\end{lstlisting}

\item \lstinline{solver=12}  (Full Stokes Matrix) cgs from linalg
\begin{lstlisting}
   sol=sps.linalg.cgs(sparse_matrix,b_fem, tol=tolerance)[0]
\end{lstlisting}

\item \lstinline{solver=13}  (Schur complement approach) {\tt SCHURCG-CG-X}

my Schur complement CG solver using cg for inner solve
\begin{lstlisting}
   solV,p,niter=schur_complement_cg_solver(K_mat,G_mat,M_mat,f_rhs,h_rhs,\
                                           NfemV,NfemP,niter_max,tolerance,use_precond,'cg')
\end{lstlisting}

\item \lstinline{solver=14} (Schur complement approach): {\tt SCHURCG-LU-X}
my Schur complement CG solver using splu for inner solve.
\begin{lstlisting}
   solV,p,niter=schur_complement_cg_solver(K_mat,G_mat,M_mat,f_rhs,h_rhs,\
                                           NfemV,NfemP,niter_max,tolerance,use_precond,'splu')
\end{lstlisting}

\item \lstinline{solver=15} (Schur complement approach): {\tt SCHURU1-DIR9ECT-X}
uzawa 1: Section 5.1 \cite{braess}, Parameterised by $\omega$.
\begin{lstlisting}
   solV,p,niter=uzawa1_solver(K_mat,G_mat,M_mat,f_rhs,h_rhs,\
                       NfemV,NfemP,niter_max,tolerance,use_precond,'direct',omega)
\end{lstlisting}

\item \lstinline{solver=16} (Schur complement approach): {\tt SCHURU2-DIRECT-X}
uzawa 2, Section 5.2 \cite{braess}. 
\begin{lstlisting}
   solV,p,niter=uzawa2_solver(K_mat,G_mat,M_mat,f_rhs,h_rhs,\
                       NfemV,NfemP,niter_max,tolerance,use_precond,'direct')
\end{lstlisting}

\item \lstinline{solver=17} (Schur complement approach): 
projection solver\footnote{where does it come from?!?}.
\begin{lstlisting}
solV,p,niter=projection_solver(K_mat,G_mat,L_mat,f_rhs,h_rhs,NfemV,NfemP,niter_max,tolerance)
\end{lstlisting}

\item \lstinline{solver=18} (Schur complement approach): {\tt SCHURU1A-DIRECT-L2PROJ}
Uzawa 1 + L2 projection.
\begin{lstlisting}
solV,p,niter=uzawa1_solver_L2(K_mat,G_mat,MP_mat,f_rhs,h_rhs,NfemP,niter_max,tolerance,omega)
\end{lstlisting}

\item \lstinline{solver=19} (Schur complement approach) {\tt SCHURU1B-DIRECT-L2PROJ}
\begin{lstlisting}
solV,p,niter=uzawa1_solver_L2b(K_mat,G_mat,MP_mat,f_rhs,h_rhs,NfemP,niter_max,tolerance,omega)
\end{lstlisting}
In this variant the solve carried out with the pressure mass matrix yields
a pressure correction, not a new pressure field as in the previous solver 18.
See further for details on the L2-based solvers.

\item \lstinline{solver=20} (Schur complement approach)  {\tt SCHURU2-DIRECT-L2PROJ}
\begin{lstlisting}
solV,p,niter=uzawa2_solver_L2(K_mat,G_mat,MP_mat,H_mat,f_rhs,h_rhs,NfemP,niter_max,tolerance)
\end{lstlisting}

\item \lstinline{solver=21} (Schur complement approach) {\tt SCHURU3-DIRECT-X}
\begin{lstlisting}
solV,p,niter=uzawa3_solver(K_mat,G_mat,f_rhs,h_rhs,NfemP,niter_max,tolerance)
\end{lstlisting}

\item \lstinline{solver=22} (Schur complement approach) {\tt SCHURU3-DIRECT-L2}
\begin{lstlisting}
solV,p,niter=uzawa3_solver_L2(K_mat,G_mat,MP_mat,H_mat,f_rhs,h_rhs,NfemP,niter_max,tolerance,'direct')
\end{lstlisting}

\item \lstinline{solver=23}  (Schur complement approach)  {\tt SCHURU3-LU-L2}
\begin{lstlisting}
...
\begin{itemize}
\item All the solvers above containing the ``$L_2$ projection'' (18 to 23) are related to the 
...
More information about the $L_2$ stuff is presented hereafter, see `About the Uzawa solvers' 
and `$L_2$ projection stuff' sections.

\item this \stone is not optimised for performance: the assembly and matrix storage could 
be improved. At the moment all matrices are stored as \lstinline{lil_matrix}.

\item Numba/jit is not used 
...
\item When relying on the Donea \& Huerta manufactured solution 
we need not discuss the preconditioning of the 
global matrix nor the $\K$ block since the viscosity is constant 
and the domain is a unit square.
...
We then discard solvers 7, 10, 12, 17 and run now the remaining ones on $48\times 48$ mesh using
...
but relying on different functions/approaches (i.e. with and without umfpack), 
while both 14 and 23 use LU decomposition.
...
To recap:
\begin{itemize}
\item Solver 1 is a direct solver provided by scipy.
\item Solver 14 is an 'old' Schur complement CG solver using splu for inner solve.
\item Solver 23 Uzawa 3 (i.e. CG on Schur), spLU inner, $L_2$ projection
\end{itemize} 
...
\includegraphics[width=10cm]{python_codes/fieldstone_147/RESULTS/timings2/solve_time3.pdf}\\
{\captionfont Results obtained with {\tt script\_errors\_1\_14\_23}, in 
{\tt fieldstone\_147/RESULTS/timings2}.}
\end{center}
...
\item
At resolution $192\times 192$ solvers 14 \& 23 are approx. 3x faster than solver 1
\end{itemize}
...
\begin{center}
\includegraphics[width=5.7cm]{python_codes/fieldstone_147/RESULTS/tol_study_solver14/errors.pdf}
\includegraphics[width=5.7cm]{python_codes/fieldstone_147/RESULTS/tol_study_solver14/solve.pdf}
\includegraphics[width=5.7cm]{python_codes/fieldstone_147/RESULTS/tol_study_solver14/solve2.pdf}\\
{\captionfont Left: Influence of the relative tolerance on the accuracy of the solution obtained 
with the SC-CG+ilu ({\python solver=14}). Middle, Right: solve time in seconds.}
\end{center}
...
The following figure shows the convergence of these solvers U1, U2 and U3 (U1 for different values of $\alpha$ parameter).
...
{\captionfont Convergence on 24x24 mesh (left) and 48x48 mesh (right) for Donea \& Huerta manufactured solution.}
\end{center}
...
{\captionfont The number of iterations required by U2 is much larger than for U3.}
\end{center}
...
\newpage
%==============================================================
\section*{$L_2$ projection - some implementation details - Donea \& Huerta}
...
Note that building and re-using the pressure mass matrix is simpler than 
computing the integral everytime, but it may not be 
the fastest approach. One could instead resort to computing the integral directly
and only assemble vectors instead of matrices. 
...
Note that in the case of the D\&H benchmark the velocity boundary conditions
are no slip on all sides, so that in practice the rhs vector $\vec{h}=0$.
...
In the context of the $L_2$ projection, the second line of the algorithm 
is replaced by a new one and e start instead from 
\[
...
\begin{itemize}
\item Solver 15 is the standard Uzawa 1 {\tt SCHURU1-DIRECT-X} ({\tt uzawa1\_solver.py}):
\begin{lstlisting}
for k in range (0,niter):
...
\item Solver 18 is Uzawa 1 with $L_2$ projection {\tt SCHURU1-DIRECT-L2}  ({\tt uzawa1\_solver\_L2.py}):
\begin{lstlisting}
for k in range (0,niter):
    solV=sps.linalg.spsolve(K_mat,f_rhs-G_mat.dot(solP))     
...
\item Solver 19 is Uzawa 1 with $L_2$ projection {\tt SCHURU1B-DIRECT-L2}in which 
...
Compared to the standard Uzawa1 approach (\lstinline{solver=15}) the L2-projection based
solvers have an additional cost, 
...
\begin{itemize}
\item Solver 16 {\tt SCHURU2-DIRECT-X} is the standard Uzawa 2:
\begin{lstlisting}
for k in range (0,niter): 
...
\item Solver 20 {\tt SCHURU2-DIRECT-L2} is Uzawa 2 with L2 projection: 
\begin{lstlisting}
for k in range (0,niter): 
...
The first residual $\vec{r}_0$ is then computed as follows {\color{red} there should probably be a mass matrix multiplying h !! - check paper}:
\begin{lstlisting}
...
is then reused for each solve with $\K$. 

\end{itemize}

This results in a drastic speedup as shown hereunder:
...
Again, the $L_2$ projection improves on the standard Uzawa solver, and in 
...
\hline
32x32=1024    & 1.748    &  7  &2.046  &   8  & 2.073    & 10 \\
40x40=1600    & 4.159    &  8  &4.557  &   8  & 4.716    & 10 \\
48*48=2304    & 8.103    &  8  &8.358  &   9  &  9.374   & 11 \\
64x64=4096    & 18.581   &  8  &20.229 &   9  &  22.501  & 11 \\
80x80=6400    & 34.032   &  8  &36.868 &   9  &  41.397  & 11 \\
96x96=9216    & 73.158   &  9  &72.908 &   9  &  84.936  & 11 \\
128x128=16384 & 192.415  &  9  &205.503&   10 &  226.167 & 11 \\
160x160=25600 &          &     &       &      &  436.069 & 12 \\
192x192=36864 &          &     &       &      &  786.035 & 12 \\
\hline
\end{tabular}
\end{center}
...
\end{center}
We find that the additional solves and matrix-vector multiplications for the $L_2$ variant (solver 22)
...
Thoughts about the $Q_1\times P_0$ element: As its name indicates the 
pressure space is made of constant values over elements. As such the 
pressure basis functions derivatives needed to compute ${\bm H}$ are zero and so is then 
${\bm H}$. I have not worked it all out but this is not a good start...


\newpage
%====================================================
\section*{Donea \& Huerta manufactured solution With preconditioner}

Before reading what follows I refer the reader to \stone~16. 
In that \stone I show (for the $Q_1 \times P_0$ element pair) that 
the preconditioner ${\M} = \G^T \cdot (\text{diag} [\K] )^{-1} \cdot \G$ is the best. 
However, building it is not that simple since it is not diagonal...
