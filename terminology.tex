\begin{flushright} {\tiny {\color{gray} terminology.tex}} \end{flushright}
...
\[
u^h(x,y) \sim a_1 + a_2 x + a_3 y + a_4 xy + a_5 x^2 + a_6 y^2
\]
with similar approximations for $v$, and 
\[
p^h(x,y) \sim b_1 + b_2x + b_3 y
\]
...
\[
u^h(x,y) \sim a_1 + a_2 x + a_3 y + a_4 xy + a_5 x^2 + a_6 y^2 + a_7 x^2y + a_8 xy^2 + a_9 x^2y^2
\]
and 
\[
p^h(x,y) \sim b_1 + b_2x + b_3 y + b_4 xy
\]
...
For instance, the rotated $Q_1 \times P_0$ element of Rannacher and Turek 
(see section \ref{ss:RTq1p0}) is such that 
