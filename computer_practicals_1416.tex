...
(please do read section 1 of \cite{wodd09} for more information, check also Plumley \& Julien (2019) 
\cite{plju19} and references therein).
...
One can resolve the stress tensor ${\bm \sigma}$ into its spherical part $-p{\bm 1}$ and its stress deviation ${\bm \tau}$ (see Eq. (\ref{steq})), where the deviatoric stress tensor is proportional to the strain rate tensor $\dot{\bm \varepsilon}$ (see Eq.(\ref{stwomu})) through the dynamic viscosity $\eta$. 
...
= \left( \frac{1}{V_\Omega} \int_\Omega (u^2+v^2) \; dV \right)^{1/2} 
\label{eq_vrms}
\end{equation}
...
are indicated in the followiug table alongside those of Blankenbach et al \cite{blbc89} 
...
\begin{tabular}{llcc}
\hline
          &           & Blankenbach et al \cite{blbc89} & Tackley \cite{tack94}    \\
\hline
\hline
...
\item {\tt bot\_bc\_noslip}: flag to switch no slip boundary conditions at bottom boundary  (default is free slip)
\end{itemize}
...
\item For three contrasting $Ra$ values, plot the temperature profiles (data to be found in {\sl T\_profile.ascii}) 
