...
This example is of course very naive and direct solvers are often very large 
...
are vastly out of the scope of this course \cite{saad}.     
...
\qquad k=0,1,\dots
\end{equation}
where $\vec{T}^k$ is the initial guess (often taken to be zero).
...
\right)
\end{equation}
We then start with the guess $\vec{T}^0=\vec{0}$, so that for $k=1$:
\begin{equation}
\vec{T}^1 = {\bm D}^{-1}\cdot \vec{b} = 
\left(
\begin{array}{ccc}
1 & 0 & 0 \\
0 & 1/5 & 0 \\
...
\begin{eqnarray}
\vec{T}^2 
&=& {\bm D}^{-1} \cdot \left[-({\bm L} + {\bm U}) \cdot \vec{T}^1 + \vec{b} \right] \nn\\
&=&
\left(
\begin{array}{ccc}
1 & 0 & 0 \\
...
$\rightarrow$ {\tt Exercise\_4\_FDM.ipynb}
