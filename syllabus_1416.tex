...
\begin{tabular}{lcl}
Continuum mechanics &$\leftrightarrow$& continuummechanica \\ 
Coordinate system &$\leftrightarrow$& coordinatenstelsel \\ 
Cartesian coordinates system &$\leftrightarrow$& cartesische coordinatenstelsel \\ 
Scalar &$\leftrightarrow$& scalair, scalaire grootheid of scalar \\ 
Vector  &$\leftrightarrow$& vector \\
Tensor  &$\leftrightarrow$& tensor \\
Axis &$\leftrightarrow$& axis \\
Unit vector &$\leftrightarrow$& eenheidsvector \\
Cross product &$\leftrightarrow$& vectorprodukt of uitprodukt\\
volume forces &$\leftrightarrow$&  volumekrachten \\
Surface forces &$\leftrightarrow$&  oppervlaktekrachten \\
Stress tensor &$\leftrightarrow$&  spanningstensor \\
\end{tabular}
\end{minipage}}
\end{center}
...
How do they compare with those of the positive faces?
}
\end{exercise}
\end{minipage}
}
...
b) Compute the material derivative of $T$ at any position and time.\\
Bonus: Compute the particle acceleration. 
}
\end{exercise}
\end{minipage}
}
...
\fbox{
\begin{minipage}{0.9\textwidth}
\includegraphics[width=0.6cm]{images/flagNL} De algemene bewegingsvergelijking 
en de continuiteitsvergelijking vormen
de basisvergelijkingen van de continuummechanica.
Elke deformeerbare stof voldoet eraan.

\begin{tabular}{lcl}
The fluid &$\leftrightarrow$& de vloiestof \\ 
The continuity equation &$\leftrightarrow$& de continuiteitsvergelijking \\
The general equation of motion &$\leftrightarrow$& de algemene bewegingsvergelijking \\
An incompressible material &$\leftrightarrow$& en onsamendrukbare stof \\
Strain rate &$\leftrightarrow$& de deformatiesnelheid \\
stress(tensor) &$\leftrightarrow$& de spanning(stensor) \\
velocity gradient &$\leftrightarrow$& \\
rotation rate &$\leftrightarrow$& \\
pressure &$\leftrightarrow$& de druk \\
The constitutive equation &$\leftrightarrow$& de constitutievergelijking\\
\end{tabular}
\end{minipage}}
\end{center}
...
a) Prove that the principal values of the deviatoric 
stress are equal in magnitude but opposite in sign by 
computing the eigen values.\\
b) As we will see later in the course, the trace of a tensor is 
invariant (i.e. unchanged) by rotation. Use this information 
to prove the previous relationship again without calculations.\\
c) Demonstrate that only shear traction exists on planes bisecting the 
principal axis of deviatoric stress
}
\end{exercise}
\end{minipage}
}
...
stresses $\sigma_i$ as: $\sigma_i'=\sigma_i +\overline{p}$
}
\end{exercise}
\end{minipage}
}
...
\todo[inline]{ADD effective viscosity formula}
...
\begin{exercise}
{\small \it
a) What is the relative change in effective viscosity 
assuming both pressure and tempoerature are increased by 10\% ?\\
b) Same question when only temperature is increased by 10\% ?}
\end{exercise}
\end{minipage}
}
\vspace{0.5cm}
...
%==============================================================
\section{Density perturbations as a driving force for mantle convection}
...
\begin{exercise}
{\small \it 
Consider an incompressible flow in three dimensions and (implicitly) define the
vector potential $\vec\psi$ as $\vec\upnu = -\vec\nabla \times \vec\psi$.\\
Assume a Newtonian fluid with constant viscosity and assume that $\rho D\upnu_i/Dt \sim 0$.\\
Demonstrate that 
\[
\vec\nabla \times
\vec\nabla \times
...
Bonus: Demonstrate the well known identity $\vec\nabla\cdot\vec\upnu=0$.
\end{exercise}
\end{minipage}
}
\vspace{0.5cm}
...
\begin{exercise}
{\small \it 
Derive equations \eqref{eq:md46a} and \eqref{eq:md46b}.
}
\end{exercise}
\end{minipage}
}
...
\begin{exercise}
{\small \it 
Derive equations \eqref{eq:md48a} and \eqref{eq:md48b}.
}
\end{exercise}
\end{minipage}
}
...
\begin{exercise}
{\small \it 
Derive Eq.~\eqref{eq:md49} and justify the assumption $p_0=0$.
}
\end{exercise}
\end{minipage}
}
...
What do each term physically account for? 
}
\end{exercise}
\end{minipage}
}
...
\frac{\partial \uppi_{ij}}{\partial x_j} + {\tilde \rho} g_i^0
\]
...
\[
\int_V {\tilde \rho} g w \; dV = \int_V \uppi_{ij} 
...
and show that the lhs of the equation equals the power due to the change in
gravitational potential energy as a result of ${\tilde \rho}$ w.r.t. $\rho_0$.
}
\end{exercise}
\end{minipage}
}
...
\begin{exercise}
{\small \it 
Derive the heat equation for a static medium ($\vec\upnu=\vec{0}$) from the conservation law of
...
\begin{exercise}
{\small \it 
Assume an isothermal (i.e. $\tilde{T}=0$) and incompressible fluid ($K_T^{-1}=0$) and
...
\begin{exercise}
{\small \it 
Show for an incompressible Newtonian fluid that at $z=0$ the traction $\vec{t}^n$ is given by
