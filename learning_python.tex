...
# generating figure with the data
plt.figure()
...
%----------------------------------
\subsection{Making simple 2d field plots }

Let us assume that we have a file {\tt temperature.ascii}
which contains three columns: the $x$-coordinate, $y$-coordinate 
and temperature $T$ at all nodes of a grid.


\begin{lstlisting}
data=np.loadtxt("temperature.ascii")
x=data[:,0]
y=data[:,1]
T=data[:,2]
x2 = np.unique(x) # remove duplicate x values
y2 = np.unique(y) # remove duplicate y values
T2 = np.reshape(T,(len(x2),len(y2)))
plt.figure()
col=plt.pcolormesh(x2,y2,T2,cmap='bwr')
plt.colorbar(col)
plt.xlabel("x")
plt.ylabel("y")
plt.show()
...
We could also have used a scatter plot (but it is not really 
what we want in the case of a regular grid):
...
T=data[:,2]
plt.figure()
col=plt.scatter(x,y,c=T,cmap='bwr')
plt.colorbar(col)
...
%------------------------------------------------
\subsection{Making simple 3d plots of a function}


\begin{flushright} {\tiny {\color{gray} provided by Jochem Veerman}} \end{flushright}
...
L = 3 # half length for x and y

x = np.linspace(-L,L,1000) # x-array
y = np.linspace(-L,L,1000) # y-array
X, Y = np.meshgrid(x,y,indexing="xy") # create 2d meshes
Z = Z_func(X,Y) # Compute function using 2d meshes
...
\begin{center}
\includegraphics[width=4cm]{images/python/parabola}
\end{center}
