...
if (ndim==2) then
   ...
   !-----------
   case('Q1Fu')
      N(1)=0.25*(1-r)*(1-s) -0.5* 0.5*(1-r)*(1-s**2)
      N(2)=0.25*(1+r)*(1-s) -0.5* 0.5*(1+r)*(1-s**2)
      N(3)=0.25*(1+r)*(1+s) -0.5* 0.5*(1+r)*(1-s**2)
      N(4)=0.25*(1-r)*(1+s) -0.5* 0.5*(1-r)*(1-s**2)
      N(5)=0.5*(1-r)*(1-s**2)
      N(6)=0.5*(1+r)*(1-s**2)
   !-----------
   case('Q1Fv')
      N(1)=0.25*(1-r)*(1-s) -0.5* 0.5*(1-r**2)*(1-s)
      N(2)=0.25*(1+r)*(1-s) -0.5* 0.5*(1-r**2)*(1-s)
      N(3)=0.25*(1+r)*(1+s) -0.5* 0.5*(1-r**2)*(1+s)
      N(4)=0.25*(1-r)*(1+s) -0.5* 0.5*(1-r**2)*(1+s)
      N(5)=0.5*(1-r**2)*(1-s)
      N(6)=0.5*(1-r**2)*(1+s)
   !-----------
   case('__Q2')
   ...
   !-----------
   case('_Q2s')
      N(1)=(1-r)*(1-s)*(-r-s-1)*0.25
      N(2)=(1+r)*(1-s)*(r-s-1) *0.25
      N(3)=(1+r)*(1+s)*(r+s-1) *0.25
      N(4)=(1-r)*(1+s)*(-r+s-1)*0.25
      N(5)=(1-r**2)*(1-s)*0.5
      N(6)=(1+r)*(1-s**2)*0.5
      N(7)=(1-r**2)*(1+s)*0.5
      N(8)=(1-r)*(1-s**2)*0.5
   ...
   !-----------
   case('__P4')
      N(01)=(3-25*r-25*s+70*r**2+140*r*s+70*s**2 -80*r**3-240*r**2*s-240*r*s**2-80*s**3 &
             +32*r**4 + 128*r**3*s + 192*r**2*s**2 + 128*r*s**3 + 32*s**4 )/3
      N(02)=(48*r -208*r**2-208*r*s +288*r**3+576*r**2*s+288*r*s**2-128*r**4- &
             384*r**3*s-384*r**2*s**2-128*r*s**3)/3
      N(03)=(-36*r +228*r**2+84*r*s -384*r**3-432*r**2*s-48*r*s**2+192*r**4+384*r**3*s+192*r**2*s**2)/3
      N(04)=(16*r -112*r**2-16*r*s +224*r**3+96*r**2*s-128*r**4-128*r**3*s)/3
      N(05)=(-3*r+22*r**2 -48*r**3+32*r**4)/3
      N(06)=(48*s -208*r*s-208*s**2 +288*r**2*s+576*r*s**2+288*s**3-128*r**3*s &
            -384*r**2*s**2-384*r*s**3-128*s**4)/3
      N(07)=(288*r*s -672*r**2*s-672*r*s**2+384*r**3*s+768*r**2*s**2+384*r*s**3)/3
      N(08)=(-96*r*s +480*r**2*s+96*r*s**2-384*r**3*s-384*r**2*s**2)/3
      N(09)=(16*r*s -96*r**2*s+128*r**3*s )/3
      N(10)=(-36*s+84*r*s+228*s**2 -48*r**2*s-432*r*s**2-384*s**3 +192*r**2*s**2+384*r*s**3+192*s**4)/3
      N(11)=(-96*r*s+96*r**2*s+480*r*s**2-384*r**2*s**2-384*r*s**3)/3
      N(12)=(12*r*s-48*r**2*s-48*r*s**2+192*r**2*s**2)/3
      N(13)=(16*s-16*r*s-112*s**2+96*r*s**2+224*s**3-128*r*s**3-128*s**4)/3
      N(14)=(16*r*s-96*r*s**2+128*r*s**3)/3
