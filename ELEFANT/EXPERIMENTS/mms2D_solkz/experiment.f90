...
real(8), parameter :: B=0.5d0*log(1.d6)  
!----------------------------------------------------------

rho=-sin(2*y)*cos(3*pi*x)
...
integer, parameter :: nnnn=3
real(8), parameter :: B=0.5d0*log(1.d6)  
real(8), parameter :: kkm=2.d0
real(8), parameter :: sigma=1.d0
real(8) stress(3),strainrate(3)

!----------------------------------------------------------

call solKz(x,y,nnnn,B,kkm,sigma,u,v,p,stress,strainrate)
...
subroutine solKz(posx,posy,n,B,km,sigma,velx,vely,presssure, total_stress,strain_rate)

implicit none

integer n;
real(8) posx,posy
real(8)  B ! /* viscosity parameter */
real(8) velx,vely, presssure
real(8) total_stress(3), strain_rate(3)
real(8) ZZZ;
real(8) u1,u2,u3,u4,u5,u6,SS;
real(8)sum1,sum2,sum3,sum4,sum5,sum6,mag,sum7,x,z;
real(8) sigma;
real(8) kn,M_PI
real(8) C1,C2,C3,C4;
real(8) Rp, UU, VV;
real(8) rho,a,bbbb,r,AA,BB,Rm,km;
real(8) t1,t2,t3,t4,t5,t6,t7,t8,t9,t10;
real(8) t11,t12,t13,t14,t15,t16,t17,t18,t19,t20;
real(8) t21,t22,t23,t24,t25,t26,t27,t28,t29,t31;
real(8) t33,t34,t35,t37,t38,t40,t41,t42,t43,t45;
real(8) t47,t51,t52,t53,t54,t55,t56,t57,t58,t59;
real(8) t60,t61,t62,t64,t65,t66,t67,t68,t69,t70;
real(8) t71,t72,t73,t74,t75,t76,t77,t78,t79,t80;
real(8) t81,t82,t83,t84,t85,t86,t89,t90,t92,t94;
real(8) t96,t97,t98,t99,t100,t101,t103,t104,t105,t106;
real(8) t107,t108,t109,t110,t111,t112,t113,t114,t115,t116;
real(8) t117,t118,t119,t120,t121,t122,t123,t124,t125,t126;
real(8) t127,t130,t131,t132,t134,t135,t141,t144,t147,t148;
real(8) t150,t151,t152,t161,t171;
...
!    /******************************************************************/
    ...
    sum5 = sum5+u5*cos(n*M_PI*x);  !/* pressure */
    u6 = u6-u5; !/* get total stress */
    sum6 = sum6+u6*cos(n*M_PI*x);  !/* xx stress */

    u1 = u1*cos(n*M_PI*x); !/* z velocity */
    sum1 = sum1+u1;
    u2 = u2*sin(n*M_PI*x); !/* x velocity */
    sum2 = sum2+u2;
    u3 = u3-u5; !/* get total stress */
    u3 = u3*cos(n*M_PI*x); !/* zz stress */
    sum3 = sum3+u3;
    u4 = u4*sin(n*M_PI*x); !/* zx stress */
    sum4 = sum4+u4;

    rho = -sigma*sin(km*z)*cos(kn*x); !/* density */
    sum7 = sum7+rho;

    SS = exp(UU*z)*(C1*cos(Rm*z)+C2*sin(Rm*z)) +exp(-VV*z)*(C3*cos(Rm*z)+C4*sin(Rm*z)) + exp(-2*z*B)*(AA*cos(km*z)+BB*sin(km*z));
    SS = SS*sin(kn*x) !/* stream function */
...
ZZZ = exp( 2.0 * B * z );
strain_rate(1) = (sum6+sum5)/(2.0*ZZZ);
strain_rate(2) = (sum3+sum5)/(2.0*ZZZ);
strain_rate(3) = (sum4)/(2.0*ZZZ);
